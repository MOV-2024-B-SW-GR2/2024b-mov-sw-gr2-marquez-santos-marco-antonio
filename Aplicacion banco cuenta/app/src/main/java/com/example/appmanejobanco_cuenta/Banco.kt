...
class Banco(
    ...
    fun obtenerCuentaPorId(numeroCuenta: Int): Cuenta {
        val indiceCuentaArreglo = this.cuentas.indexOfFirst { it.obtenerNumeroCuenta() == numeroCuenta }
        if (indiceCuentaArreglo == -1) throw Exception("No existe la cuenta...")
        return cuentas[indiceCuentaArreglo]
    }
}
